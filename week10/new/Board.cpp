...
bool Board::checkRowOrColWin(char player) 
{
	...
	for (int i = 0; i <= 2; i++) 
	{
		if ((cBoard[i][0] != '.') && (cBoard[i][0] == cBoard[i][1]) && (cBoard[i][0] == cBoard[i][2])) 
			{
...
bool Board::checkDgnWin(char player) 
{
	...
	if ((cBoard[1][1] != '.') && (((cBoard[0][0] == cBoard[1][1]) && (cBoard[0][0] == cBoard[2][2])) ||
	   ((cBoard[2][0] == cBoard[1][1]) &&	(cBoard[2][0] == cBoard[0][2])))) 
	{
